...
export class Mining extends Phaser.Scene {
    ...
    create(config)
    {
        ...
        // CREATE PLAYER
        this.pShip = new Ship(config.shipClass ? config.shipClass : config.configMana.ships[0], this, 400, 500);
...
export class MiningStatus extends Phaser.Scene {
    ...
    updateHealth() {
        if (this.pShip) { //don't update when ship is not there yet
            this.healthBar.setCrop(0, 0, 156*(this.pShip.health / this.pShip.getMaxHealth()), 16);
            this.shieldBar.setCrop(0, 0, 156*(this.pShip.shield / this.pShip.getMaxShield()), 16);
        }
    }
}
